...
1. LOAD the model with attn_implementation="eager"
   - "eager" is slower but returns the raw attention matrices needed

2. For each sample in the JSONL dataset:
   a. Build the input (system prompt + optional images + user question)
   b. Tokenize 
   c. prefill: Run the full prompt through the model in one forward pass
      - This processes all input tokens through all 26 layers
      - Returns: first next-token prediction + prefill attention
      - use prefill attention to identify sink tokens
   d. decodee: Generate tokens one at a time 
   e. segment the input into: image tokens, text tokens, sink tokens
   f. For each generated token, at each of the 4 global layers:
      - Sum attention to image positions → image_ratio
      - Sum attention to non-sink text positions → text_ratio
      - Sum attention to sink positions → sink_ratio


=============================================================================
MEDGEMMA 4B ARCHITECTURE 
=============================================================================

- 26 decoder layers, 8 attention heads per layer (4 KV heads via GQA)

- Only global layers (5, 11, 17, 23) can attend to image tokens from
  output positions (because image tokens are typically far away in the
  sequence, beyond the 1024-token local window).
...
# MedGemma 4B (Gemma 3) architecture constants
NUM_LAYERS = 26
NUM_HEADS = 8
GLOBAL_LAYER_INDICES = [5, 11, 17, 23]  # Layers that see the full sequence
...
def find_token_segments(
    ...
    """Identify which positions are image vs text tokens."""
...
def plot_full_generation_heatmap(
    ...
    for i, (title, cmap) in enumerate(zip(titles, cmaps)):
        ...
        ax.set_xticklabels([f'L{l}\n(global)' for l in GLOBAL_LAYER_INDICES], fontsize=9)
...
def main():
    ...
    ap.add_argument("--heatmap-dir", required=True,
                     help="Directory to save attention heatmap PNGs")
    ap.add_argument(
        "--mode",
...
"""
python medgemma_pertoken_attn.py \
  --jsonl train_2.jsonl.v1.jsonl \
  --images-root images_train \
  --output train_results_v1.jsonl \
  --output_attn train_results_attn.jsonl \
  --version v1 \
  --heatmap-dir ./heatmaps_multi \
  --mode multi

python medgemma_pertoken_attn.py \
  --jsonl train_2.jsonl.v2.jsonl \
  --images-root images_train \
  --output train_results_v2.jsonl \
  --output_attn train_results_attn.jsonl \
  --version v2 \
  --heatmap-dir ./heatmaps_multi \
  --mode multi
...
python medgemma_pertoken_attn.py \
  --jsonl train_2.jsonl.v3.jsonl \
  --images-root images_train \
  --output train_results_v3.jsonl \
  --output_attn train_results_attn.jsonl \
  --version v3 \
  --heatmap-dir ./heatmaps_multi \
  --mode multi
